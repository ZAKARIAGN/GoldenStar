...
import { updateCombo } from '../Services/CombosServices';
import { toast } from "react-toastify";
import ValidationErrorMsg from "../../Menu/Components/ValidationErrorMsg";
const UpdateFormSection = ({ selectedCombo, openUpdateForm, setOpenUpdateForm }) => {
  ...
  return (
    <form onSubmit={HandleSubmit} className={`${openUpdateForm ? "w-[375px] opacity-100 p-4 border border-gray-200" : "w-0 opacity-0 p-0 border-0 pointer-events-none"} transition-all duration-300 overflow-hidden min-h-[603px] rounded-xl bg-white shadow-sm`}>
      {/* Header */}
      ...
      <div className="flex gap-8">
        ...
        <button type='submit' disabled={mutation.isPending} className="h-9 flex-[1.45] rounded-lg bg-orange-600 text-[12px] font-medium text-white transition hover:bg-orange-700 cursor-pointer">
          {mutation.isPending ? "Updating..." : "Update Combo"}
        </button>
      </div>
    </form>
  )
